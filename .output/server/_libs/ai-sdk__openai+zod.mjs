...
import { At as union, C as createProviderDefinedToolFactory, Ct as literal, Dt as record, E as createToolNameMapping, Et as object, F as getTopLevelMediaType, Ft as InvalidPromptError, G as lazySchema, J as mediaTypeToExtension, K as loadApiKey, Lt as TooManyEmbeddingValuesForCallError, Mt as _coercedNumber, N as generateId, Pt as APICallError, Q as postJsonToApi, R as isCustomReasoning, S as createJsonResponseHandler, St as lazy, T as createProviderExecutedToolFactory, Tt as number$1, V as isNonNullable, X as parseProviderOptions, Y as parseJSON, Z as postFormDataToApi, _t as any, at as serializeModelOptions, c as StreamingToolCallTracker, ct as withUserAgentSuffix, dt as WORKFLOW_DESERIALIZE, f as combineHeaders, ft as WORKFLOW_SERIALIZE, g as convertToFormData, gt as _null, h as convertToBase64, jt as unknown, k as downloadBlob, kt as string, lt as withoutTrailingSlash, m as convertInlineFileDataToUint8Array, mt as _enum, nt as resolveProviderReference, p as convertBase64ToUint8Array, pt as ZodNumber, q as loadOptionalSetting, st as validateTypes, tt as resolveFullMediaType, ut as zodSchema, v as createBinaryResponseHandler, vt as array, w as createProviderDefinedToolFactoryWithOutputSchema, x as createJsonErrorResponseHandler, xt as discriminatedUnion, y as createEventSourceResponseHandler, yt as boolean, zt as UnsupportedFunctionalityError } from "./@ai-sdk/gateway+[...].mjs";
...
var OpenAIFiles = class {
	...
	async uploadFile({ data, mediaType, filename, providerOptions }) {
		...
		if ((openaiOptions == null ? void 0 : openaiOptions.expiresAfter) != null) formData.append("expires_after", String(openaiOptions.expiresAfter));
		...
		case "tool":
			for (const part of content) {
				...
				if (output.type === "execution-denied") {
					if ((_v = (_u = output.providerOptions) == null ? void 0 : _u.openai) == null ? void 0 : _v.approvalId) continue;
				}
				const resolvedToolName = toolNameMapping.toProviderToolName(part.toolName);
...
var OpenAIResponsesLanguageModel = class _OpenAIResponsesLanguageModel {
	...
	async getArgs({ maxOutputTokens, temperature, stopSequences, topP, topK, presencePenalty, frequencyPenalty, seed, prompt, reasoning, providerOptions, tools, toolChoice, responseFormat }) {
		...
		const baseArgs = {
			...
			...isReasoningModel && (resolvedReasoningEffort != null || resolvedReasoningSummary != null) && { reasoning: {
		...
		for (const part of response.output) switch (part.type) {
			...
			case "message":
				for (const contentPart of part.content) {
					if (((_h = (_g = options.providerOptions) == null ? void 0 : _g[providerOptionsName]) == null ? void 0 : _h.logprobs) && contentPart.logprobs) logprobs.push(contentPart.logprobs);
